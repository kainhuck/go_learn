...
func main() {
	// 创建临时文件最简单的方法是调用 ioutil.TempFile 函数。 它会创建并打开文件，我们可以对文件进行读写。
	// 如果函数的第一个参数传 ""，ioutil.TempFile 会在操作系统的默认位置下创建该文件。
	...
	// ioutil.TempDir 的参数与 TempFile 相同， 但是它返回的是一个 目录名 ，而不是一个打开的文件
